...
# print(CAFFE_ROOT)
CAFFE_ROOT = '/media/xzgz/Ubuntu/Ubuntu/ICPR/warpctc-caffe'
...
LABEL_SEQ_LEN = 5
# captcha images list
images = list(filter(lambda x: os.path.splitext(x)[1] == '.png', os.listdir(img_path)))
...
def write_image_info_into_hdf5(file_name, images, phase):
    ...
    def process(file_name, images):
        ...
        for i, image in enumerate(images):
            ...
            numbers = np.array(list(map(lambda x: float(x), numbers_str)))
    ...
    with open(file_name, 'w') as f:
        ...
        for p in process_pool:
            p.join()
trainning_size = 80000 # 800 # number of images for trainning
