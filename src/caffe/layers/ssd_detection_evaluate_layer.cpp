...
#include "caffe/layers/ssd_detection_evaluate_layer.hpp"
...
void DetectionEvaluateLayer<Dtype>::LayerSetUp(
  ...
  const SSDDetectionEvaluateParameter& ssd_detection_evaluate_param =
      this->layer_param_.ssd_detection_evaluate_param();
  CHECK(ssd_detection_evaluate_param.has_num_classes())
      << "Must provide num_classes.";
  num_classes_ = ssd_detection_evaluate_param.num_classes();
  background_label_id_ = ssd_detection_evaluate_param.background_label_id();
  overlap_threshold_ = ssd_detection_evaluate_param.overlap_threshold();
  CHECK_GT(overlap_threshold_, 0.) << "overlap_threshold must be non negative.";
  evaluate_difficult_gt_ = ssd_detection_evaluate_param.evaluate_difficult_gt();
  if (ssd_detection_evaluate_param.has_name_size_file()) {
    string name_size_file = ssd_detection_evaluate_param.name_size_file();
  ...
  has_resize_ = ssd_detection_evaluate_param.has_resize_param();
  if (has_resize_) {
    resize_param_ = ssd_detection_evaluate_param.resize_param();
  }
}
