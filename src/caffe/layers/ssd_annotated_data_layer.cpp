...
#include "caffe/layers/ssd_annotated_data_layer.hpp"
#include "caffe/util/benchmark.hpp"
#include "caffe/util/ssd_sampler.hpp"
...
void AnnotatedDataLayer<Dtype>::DataLayerSetUp(
  ...
  const SSDAnnotatedDataParameter& anno_data_param =
      this->layer_param_.ssd_annotated_data_param();
  for (int i = 0; i < anno_data_param.batch_sampler_size(); ++i) {
    batch_samplers_.push_back(anno_data_param.batch_sampler(i));
  }
  label_map_file_ = anno_data_param.label_map_file();
  // Make sure dimension is consistent within batch.
  const SSDTransformationParameter& ssd_transform_param =
    this->layer_param_.ssd_transform_param();
  if (ssd_transform_param.has_resize_param()) {
    if (ssd_transform_param.resize_param().resize_mode() ==
  ...
  if (this->output_labels_) {
    ...
    if (has_anno_type_) {
      ...
      if (anno_data_param.has_anno_type()) {
        // If anno_type is provided in SSDAnnotatedDataParameter, replace
...
void AnnotatedDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (ssd_transform_param.has_distort_param()) {
      distort_datum.CopyFrom(anno_datum);
      this->data_transformer_->DistortImage(anno_datum.datum(),
                                            distort_datum.mutable_datum());
      if (ssd_transform_param.has_expand_param()) {
