...
#include "caffe/layers/ssd_video_data_layer.hpp"
...
void VideoDataLayer<Dtype>::DataLayerSetUp(
  ...
  const SSDVideoDataParameter& ssd_video_data_param =
      this->layer_param_.ssd_video_data_param();
  video_type_ = ssd_video_data_param.video_type();
  skip_frames_ = ssd_video_data_param.skip_frames();
  CHECK_GE(skip_frames_, 0);

  // Read an image, and use it to initialize the top blob.
  cv::Mat cv_img;
  if (video_type_ == SSDVideoDataParameter_VideoType_WEBCAM) {
    const int device_id = ssd_video_data_param.device_id();
    if (!cap_.open(device_id)) {
      LOG(FATAL) << "Failed to open webcam: " << device_id;
    }
    cap_ >> cv_img;
  } else if (video_type_ == SSDVideoDataParameter_VideoType_VIDEO) {
    CHECK(ssd_video_data_param.has_video_file()) << "Must provide video file!";
    const string& video_file = ssd_video_data_param.video_file();
