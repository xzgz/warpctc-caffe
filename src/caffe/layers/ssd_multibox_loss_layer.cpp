...
#include "caffe/layers/ssd_multibox_loss_layer.hpp"
...
void MultiBoxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const SSDMultiBoxLossParameter& ssd_multibox_loss_param =
      this->layer_param_.ssd_multibox_loss_param();
  ssd_multibox_loss_param_ = this->layer_param_.ssd_multibox_loss_param();

  num_ = bottom[0]->num();
  num_priors_ = bottom[2]->height() / 4;
  // Get other parameters.
  CHECK(ssd_multibox_loss_param.has_num_classes()) << "Must provide num_classes.";
  num_classes_ = ssd_multibox_loss_param.num_classes();
  CHECK_GE(num_classes_, 1) << "num_classes should not be less than 1.";
  share_location_ = ssd_multibox_loss_param.share_location();
  loc_classes_ = share_location_ ? 1 : num_classes_;
  background_label_id_ = ssd_multibox_loss_param.background_label_id();
  use_difficult_gt_ = ssd_multibox_loss_param.use_difficult_gt();
  mining_type_ = ssd_multibox_loss_param.mining_type();
  if (ssd_multibox_loss_param.has_do_neg_mining()) {
    LOG(WARNING) << "do_neg_mining is deprecated, use mining_type instead.";
    do_neg_mining_ = ssd_multibox_loss_param.do_neg_mining();
    CHECK_EQ(do_neg_mining_,
             mining_type_ != SSDMultiBoxLossParameter_MiningType_NONE);
  }
  do_neg_mining_ = mining_type_ != SSDMultiBoxLossParameter_MiningType_NONE;
  ...
  loc_weight_ = ssd_multibox_loss_param.loc_weight();
  loc_loss_type_ = ssd_multibox_loss_param.loc_loss_type();
  // fake shape.
  ...
  if (loc_loss_type_ == SSDMultiBoxLossParameter_LocLossType_L2) {
  ...
  } else if (loc_loss_type_ == SSDMultiBoxLossParameter_LocLossType_SMOOTH_L1) {
    LayerParameter layer_param;
  ...
  conf_loss_type_ = ssd_multibox_loss_param.conf_loss_type();
  ...
  if (conf_loss_type_ == SSDMultiBoxLossParameter_ConfLossType_SOFTMAX) {
  ...
  } else if (conf_loss_type_ == SSDMultiBoxLossParameter_ConfLossType_LOGISTIC) {
    LayerParameter layer_param;
...
void MultiBoxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  FindMatches(all_loc_preds, all_gt_bboxes, prior_bboxes, prior_variances,
              ssd_multibox_loss_param_, &all_match_overlaps, &all_match_indices_);
  ...
  MineHardExamples(*bottom[1], all_loc_preds, all_gt_bboxes, prior_bboxes,
                   prior_variances, all_match_overlaps, ssd_multibox_loss_param_,
  ...
  if (num_matches_ >= 1) {
    ...
    EncodeLocPrediction(all_loc_preds, all_gt_bboxes, all_match_indices_,
                        prior_bboxes, prior_variances, ssd_multibox_loss_param_,
  ...
  if (num_conf_ >= 1) {
    ...
    EncodeConfPrediction(conf_data, num_, num_priors_, ssd_multibox_loss_param_,
