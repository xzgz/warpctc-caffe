...
#include "caffe/layers/ssd_detection_output_layer.hpp"
...
void DetectionOutputLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const SSDDetectionOutputParameter& ssd_detection_output_param =
      this->layer_param_.ssd_detection_output_param();
  CHECK(ssd_detection_output_param.has_num_classes()) << "Must specify num_classes";
  num_classes_ = ssd_detection_output_param.num_classes();
  share_location_ = ssd_detection_output_param.share_location();
  num_loc_classes_ = share_location_ ? 1 : num_classes_;
  background_label_id_ = ssd_detection_output_param.background_label_id();
  code_type_ = ssd_detection_output_param.code_type();
  variance_encoded_in_target_ =
      ssd_detection_output_param.variance_encoded_in_target();
  keep_top_k_ = ssd_detection_output_param.keep_top_k();
  confidence_threshold_ = ssd_detection_output_param.has_confidence_threshold() ?
      ssd_detection_output_param.confidence_threshold() : -FLT_MAX;
  // Parameters used in nms.
  nms_threshold_ = ssd_detection_output_param.nms_param().nms_threshold();
  CHECK_GE(nms_threshold_, 0.) << "nms_threshold must be non negative.";
  eta_ = ssd_detection_output_param.nms_param().eta();
  CHECK_GT(eta_, 0.);
  CHECK_LE(eta_, 1.);
  top_k_ = -1;
  if (ssd_detection_output_param.nms_param().has_top_k()) {
    top_k_ = ssd_detection_output_param.nms_param().top_k();
  }
  const SaveOutputParameter& save_output_param =
      ssd_detection_output_param.save_output_param();
  ...
  name_count_ = 0;
  visualize_ = ssd_detection_output_param.visualize();
  if (visualize_) {
    visualize_threshold_ = 0.6;
    if (ssd_detection_output_param.has_visualize_threshold()) {
      visualize_threshold_ = ssd_detection_output_param.visualize_threshold();
    }
    data_transformer_.reset(
        new SSDDataTransformer<Dtype>(this->layer_param_.ssd_transform_param(),
                                   this->phase_));
    data_transformer_->InitRand();
    save_file_ = ssd_detection_output_param.save_file();
  }
