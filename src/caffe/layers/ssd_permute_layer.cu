...
__global__ void SSDPermuteKernel(const int nthreads,
...
template <typename Dtype>
void SSDPermuteLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (need_permute_) {
    ...
    SSDPermuteKernel<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
...
template <typename Dtype>
void SSDPermuteLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(SSDPermuteLayer);
