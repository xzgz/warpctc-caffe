...
#include <vector>
#include "caffe/layers/ssd_permute_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void SSDPermute(const int count, Dtype* bottom_data, const bool forward,
...
template <typename Dtype>
void SSDPermuteLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  SSDPermuteParameter ssd_permute_param = this->layer_param_.ssd_permute_param();
  CHECK_EQ(bottom.size(), 1);
  num_axes_ = bottom[0]->num_axes();
  vector<int> orders;
  // Push the specified new orders.
  for (int i = 0; i < ssd_permute_param.order_size(); ++i) {
    int order = ssd_permute_param.order(i);
...
template <typename Dtype>
void SSDPermuteLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void SSDPermuteLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  if (need_permute_) {
    ...
    bool forward = true;
    SSDPermute(top_count, bottom_data, forward, permute_order, old_steps,
...
template <typename Dtype>
void SSDPermuteLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (need_permute_) {
    ...
    const int* new_steps = new_steps_.cpu_data();
    bool forward = false;
    SSDPermute(top_count, bottom_diff, forward, permute_order, old_steps,
...
#ifdef CPU_ONLY
STUB_GPU(SSDPermuteLayer);
#endif

INSTANTIATE_CLASS(SSDPermuteLayer);
REGISTER_LAYER_CLASS(SSDPermute);
