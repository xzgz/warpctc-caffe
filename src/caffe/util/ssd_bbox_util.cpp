...
#include "caffe/util/ssd_bbox_util.hpp"
...
void EncodeBBox(
  ...
  if (code_type == SSDPriorBoxParameter_CodeType_CORNER) {
  ...
  } else if (code_type == SSDPriorBoxParameter_CodeType_CENTER_SIZE) {
  ...
  } else if (code_type == SSDPriorBoxParameter_CodeType_CORNER_SIZE) {
    float prior_width = prior_bbox.xmax() - prior_bbox.xmin();
    CHECK_GT(prior_width, 0);
    float prior_height = prior_bbox.ymax() - prior_bbox.ymin();
    CHECK_GT(prior_height, 0);
...
void MatchBBox(const vector<NormalizedBBox>& gt_bboxes,
  ...
  switch (match_type) {
    case SSDMultiBoxLossParameter_MatchType_BIPARTITE:
      // Already done.
      break;
    case SSDMultiBoxLossParameter_MatchType_PER_PREDICTION:
...
void FindMatches(const vector<LabelBBox>& all_loc_preds,
      ...
      const vector<NormalizedBBox>& prior_bboxes,
      const vector<vector<float> >& prior_variances,
      const SSDMultiBoxLossParameter& ssd_multibox_loss_param,
      vector<map<int, vector<float> > >* all_match_overlaps,
      vector<map<int, vector<int> > >* all_match_indices) {
  // all_match_overlaps->clear();
  // all_match_indices->clear();
  // Get parameters.
  CHECK(ssd_multibox_loss_param.has_num_classes()) << "Must provide num_classes.";
  const int num_classes = ssd_multibox_loss_param.num_classes();
  CHECK_GE(num_classes, 1) << "num_classes should not be less than 1.";
  const bool share_location = ssd_multibox_loss_param.share_location();
  const int loc_classes = share_location ? 1 : num_classes;
  const MatchType match_type = ssd_multibox_loss_param.match_type();
  const float overlap_threshold = ssd_multibox_loss_param.overlap_threshold();
  const bool use_prior_for_matching =
      ssd_multibox_loss_param.use_prior_for_matching();
  const int background_label_id = ssd_multibox_loss_param.background_label_id();
  const CodeType code_type = ssd_multibox_loss_param.code_type();
  const bool encode_variance_in_target =
      ssd_multibox_loss_param.encode_variance_in_target();
  const bool ignore_cross_boundary_bbox =
      ssd_multibox_loss_param.ignore_cross_boundary_bbox();
  // Find the matches.
...
inline bool IsEligibleMining(const MiningType mining_type, const int match_idx,
  ...
  if (mining_type == SSDMultiBoxLossParameter_MiningType_MAX_NEGATIVE) {
    return match_idx == -1 && match_overlap < neg_overlap;
  } else if (mining_type == SSDMultiBoxLossParameter_MiningType_HARD_EXAMPLE) {
    return true;
  } else {
    return false;
  }
...
void MineHardExamples(const Blob<Dtype>& conf_blob,
  ...
  *num_negs = 0;
  ...
  CHECK_GE(num_classes, 1) << "num_classes should not be less than 1.";
  const int background_label_id = ssd_multibox_loss_param.background_label_id();
  const bool use_prior_for_nms = ssd_multibox_loss_param.use_prior_for_nms();
  const ConfLossType conf_loss_type = ssd_multibox_loss_param.conf_loss_type();
  const MiningType mining_type = ssd_multibox_loss_param.mining_type();
  if (mining_type == SSDMultiBoxLossParameter_MiningType_NONE) {
    return;
  }
  const LocLossType loc_loss_type = ssd_multibox_loss_param.loc_loss_type();
  const float neg_pos_ratio = ssd_multibox_loss_param.neg_pos_ratio();
  const float neg_overlap = ssd_multibox_loss_param.neg_overlap();
  const CodeType code_type = ssd_multibox_loss_param.code_type();
  const bool encode_variance_in_target =
      ssd_multibox_loss_param.encode_variance_in_target();
  const bool has_nms_param = ssd_multibox_loss_param.has_nms_param();
  float nms_threshold = 0;
  int top_k = -1;
  if (has_nms_param) {
    nms_threshold = ssd_multibox_loss_param.nms_param().nms_threshold();
    top_k = ssd_multibox_loss_param.nms_param().top_k();
  }
  const int sample_size = ssd_multibox_loss_param.sample_size();
  ...
  if (mining_type == SSDMultiBoxLossParameter_MiningType_HARD_EXAMPLE) {
    ...
    if (*num_matches != 0) {
      ...
      EncodeLocPrediction(all_loc_preds, all_gt_bboxes, *all_match_indices,
                          prior_bboxes, prior_variances, ssd_multibox_loss_param,
...
void EncodeLocPrediction(const vector<LabelBBox>& all_loc_preds,
  ...
  const bool encode_variance_in_target =
      ssd_multibox_loss_param.encode_variance_in_target();
  const bool bp_inside = ssd_multibox_loss_param.bp_inside();
  const bool use_prior_for_matching =
      ssd_multibox_loss_param.use_prior_for_matching();
  int count = 0;
  for (int i = 0; i < num; ++i) {
    ...
    for (map<int, vector<int> >::const_iterator
      ...
      for (int j = 0; j < match_index.size(); ++j) {
        ...
        for (int k = 0; k < 4; ++k) {
          ...
          if (loc_loss_type == SSDMultiBoxLossParameter_LocLossType_SMOOTH_L1) {
          ...
          } else if (loc_loss_type == SSDMultiBoxLossParameter_LocLossType_L2) {
            loss += 0.5 * val * val;
          } else {
...
void ComputeConfLoss(const Dtype* conf_data, const int num,
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int p = 0; p < num_preds_per_class; ++p) {
      ...
      Dtype loss = 0;
      if (loss_type == SSDMultiBoxLossParameter_ConfLossType_SOFTMAX) {
        CHECK_GE(label, 0);
      ...
      } else if (loss_type == SSDMultiBoxLossParameter_ConfLossType_LOGISTIC) {
        int target = 0;
        for (int c = 0; c < num_classes; ++c) {
          if (c == label) {
            target = 1;
...
void EncodeConfPrediction(const Dtype* conf_data, const int num,
      const int num_priors, const SSDMultiBoxLossParameter& ssd_multibox_loss_param,
      const vector<map<int, vector<int> > >& all_match_indices,
  ...
  const bool map_object_to_agnostic =
      ssd_multibox_loss_param.map_object_to_agnostic();
  ...
  const MiningType mining_type = ssd_multibox_loss_param.mining_type();
  bool do_neg_mining;
  if (ssd_multibox_loss_param.has_do_neg_mining()) {
    LOG(WARNING) << "do_neg_mining is deprecated, use mining_type instead.";
    do_neg_mining = ssd_multibox_loss_param.do_neg_mining();
    CHECK_EQ(do_neg_mining,
             mining_type != SSDMultiBoxLossParameter_MiningType_NONE);
  }
  do_neg_mining = mining_type != SSDMultiBoxLossParameter_MiningType_NONE;
  const ConfLossType conf_loss_type = ssd_multibox_loss_param.conf_loss_type();
  int count = 0;
  for (int i = 0; i < num; ++i) {
    if (all_gt_bboxes.find(i) != all_gt_bboxes.end()) {
      ...
      for (map<int, vector<int> >::const_iterator it =
        ...
        for (int j = 0; j < num_priors; ++j) {
          ...
          switch (conf_loss_type) {
            case SSDMultiBoxLossParameter_ConfLossType_SOFTMAX:
              conf_gt_data[idx] = gt_label;
              break;
            case SSDMultiBoxLossParameter_ConfLossType_LOGISTIC:
