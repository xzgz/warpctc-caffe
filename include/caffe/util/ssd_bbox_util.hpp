...
typedef SSDPriorBoxParameter_CodeType CodeType;
typedef SSDMultiBoxLossParameter_MatchType MatchType;
typedef SSDMultiBoxLossParameter_LocLossType LocLossType;
typedef SSDMultiBoxLossParameter_ConfLossType ConfLossType;
typedef SSDMultiBoxLossParameter_MiningType MiningType;
...
void FindMatches(const vector<LabelBBox>& all_loc_preds,
      ...
      const vector<NormalizedBBox>& prior_bboxes,
      const vector<vector<float> >& prior_variances,
      const SSDMultiBoxLossParameter& multibox_loss_param,
...
void EncodeConfPrediction(const Dtype* conf_data, const int num,
      const int num_priors, const SSDMultiBoxLossParameter& multibox_loss_param,
      const vector<map<int, vector<int> > >& all_match_indices,
